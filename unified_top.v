// Unified top-level module combining VGA and Keyboard functionality
module hfrisc_soc
(
	input wire clk_i,
	input wire rst_i,
	...
	// External peripheral signals for both VGA and keyboard
	wire ext_periph_vga; reg ext_periph_vga_dly;
	wire ext_periph_kbd; reg ext_periph_kbd_dly;
	wire ext_periph;
	reg ext_periph_dly;
	...
	// Keyboard/AXIS interface signals  
	wire reset_n;
	...
	always @(posedge clock, posedge rst_i) begin
		if (rst_i == 1'b1) begin
			...
			ext_periph_kbd_dly <= 1'b0;
			ext_periph_dly <= 1'b0;
		end else begin
			ram_dly <=  ~ram_enable_n;
			periph_dly <= periph;
			ext_periph_vga_dly <= ext_periph_vga;
			ext_periph_kbd_dly <= ext_periph_kbd;
	...
	// Combine external peripheral access signals
	...
	// AXI stream / CPU interface core for keyboard
	if_axis if_axis_core1
	(
		// external CPU interface signals
		.addr_i(address),
		.data_o(data_read_axis_s),
		.data_access_o(ext_periph_kbd),
		.data_w_i(periph_wr),
		// AXI stream interface signals
		.axis_aclk_i(clock),
		.axis_aresetn_i(reset_n),
		// slave axi stream interface (connected to keyboard axi master interface)
